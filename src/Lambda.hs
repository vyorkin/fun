...
-- All closed lambda expressions can be expressed
-- in terms of only the S and K combinators
...
-- Substitution: `(\x.e)a -> [x/a] e`
-- means replace all free occurrences of
-- variable `x` in `e` with `a`
...
-- Eta-expansion:
-- The opposite of the eta-reduction
